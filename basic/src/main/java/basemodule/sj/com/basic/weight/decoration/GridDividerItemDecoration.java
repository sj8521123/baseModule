package basemodule.sj.com.basic.weight.decoration;
...
//用法：mRecycleView.addItemDecoration(new GridDividerItemDecoration(this, ScreenUtil.dp2px(2), ScreenUtil.dp2px(2), ScreenUtil.dp2px(2), true, true, Color.RED));
public class GridDividerItemDecoration extends RecyclerView.ItemDecoration {
    private Paint mPaint;
    //竖直方向：第一列左，最后一列右 + item竖直之间 + 最后一行底部
    private int mDividerVertical;          //您所需指定的间隔宽度，主要为第一列和最后一列与父控件的间隔；行间距，列间距将动态分配
    //item水平之间的间距
    ...
    public GridDividerItemDecoration(Context context, int mDividerVertical, int mDividerHorizontal, int firstRowTopMargin, boolean isNeedSpace, boolean isLastRowNeedSpace, @ColorInt int color) {
        this.mDividerVertical = mDividerVertical;
        this.mDividerHorizontal = mDividerHorizontal;
    ...
    private int getMaxDividerWidth(View view) {
        ...
        if (itemHeight < 0 || itemWidth < 0 || (isNeedSpace && maxDividerWidth <= (spanCount - 1) * mDividerVertical)) {
    ...
    private int getAttachCloumnWidth() {
        ...
        try {
            int width = mContext.getResources().getDisplayMetrics().widthPixels > mContext.getResources().getDisplayMetrics().heightPixels
                    ? mContext.getResources().getDisplayMetrics().heightPixels : mContext.getResources().getDisplayMetrics().widthPixels;
            if (isNeedSpace)
                spaceWidth = 2 * mDividerVertical;
            itemWidth = (width - spaceWidth) / spanCount - mDividerHorizontal;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void draw(Canvas canvas, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            int bottom = top + mDividerVertical;
            if (mPaint != null) {
                canvas.drawRect(left, top, right, bottom, mPaint);
            }
            //画垂直分割线
            top = child.getTop();
            bottom = child.getBottom() + mDividerVertical;
            left = child.getRight() + layoutParams.rightMargin;
            right = left + mDividerVertical;
            if (mPaint != null) {
                canvas.drawRect(left, top, right, bottom, mPaint);
            }
        }
